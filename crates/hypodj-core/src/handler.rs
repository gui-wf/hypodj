...
impl HypodjHandler {
    ...
    pub async fn advance_on_eof(&self) {
        let next = {
            ...
            // EOF inside that window. Advancing here would load an unrelated track
            // (current+1) and collide with the pending Terminal::SkipLoad, which
            // still fires afterward and loads the skip target a second time - a
            // spurious load plus an audible double-load glitch. Leave the advance to
            // the skip terminal.
    ...
    async fn handle(&self, cmd: MpdCommand) -> MpdResponse {
        match cmd {
            ...
            MpdCommand::Next => {
                let next = {
                    let st = self.state.lock().unwrap();
                    // Advance from the REPORTED current: during an in-flight skip
                    // the user already sees the target as current, so a second Next
                    // steps past IT (idx+1), not the still-loaded old track.
                    st.reported_current().map(|c| c + 1).filter(|&i| i < st.queue.len())
                };
                match next {
