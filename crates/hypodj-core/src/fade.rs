...
/// too-short requested duration UP (never a hard cut). `min_slew` is the per-step
/// interval floor; `synth_floor_db` resolves a `Silence` target to its ramp end.
